...
def fit_font_to_width(draw: ImageDraw.Draw,
                      ...
                      start_size: int = 200,
                      min_size: int   = 10,
                      step: int       = 2) -> ImageFont.ImageFont:
    """
...
def create_MplusImage(active_run, run, donesocials, check_socials=True, add_region=True, add_season=True):
    ...
    members = sorted(
        members,
        key=lambda m: (
            int(spec_lookup[str(m["spec_id"])]["role"]),  
            int(m["spec_id"])                              
        )
    )
    out_path = os.path.join(OUTPUT_DIR, f"{run}_mplus_{dungeon_id}_{level}_{duration_ms}_{timestamp}.png")
    ...
    w = bbox[2] - bbox[0]
    h = bbox[3] - bbox[1]
    draw.text(((WIDTH - w) // 2, HEIGHT - h - 20), footer_text, font=footer_font, fill=(180,180,180))
    ...
    for idx, member in enumerate(members):
        ...
        cy = y + 50
        color = (int(class_info["color"]["r"]), int(class_info["color"]["g"]), int(class_info["color"]["b"]))
        draw.rectangle((cx-45, cy-45, cx+45, cy+45), outline=color, width=6)

        # paste spec icon
        canvas.paste(icon_img, (cx-40, cy-40), icon_img if icon_img.mode=='RGBA' else None)
        ...
        tw = bbox[2] - bbox[0]
        th = bbox[3] - bbox[1]
        draw.text((cx - tw/2, cy + 50), txt, font=small_font, fill=color, stroke_width=2, stroke_fill=(0, 0, 0))
...
def get_run_data(run_type, spec, season):
    with closing(databaseConnector.get_connection()) as conn:
        cursor = conn.cursor()
        if spec:
            return databaseConnector.fetch_max_key_run_per_spec(conn, cursor, spec, season)
        if run_type == 'longest':
...
def create_overall_spec_popularity(output_dir, donesocials, api_key, url, season, icon_size=0.4):
    """
    ...
    with closing(databaseConnector.get_connection()) as conn:
        ...
        dungeon_runs_per_level = databaseConnector.fetch_runs_per_dungeon_per_level(conn, cursor, season)
    ...
    if not dungeon_runs_per_level:
        ...
        ddf = pd.DataFrame([{'id': k, 'count': v} for k, v in d_groups.items()]).sort_values('count', ascending=False).reset_index(drop=True)
    ...
    for _, row in df.iterrows():
        ...
        draw = ImageDraw.Draw(canvas)
        draw.rectangle(
            [0, 0, canvas_s-1, canvas_s-1],
            outline=color_rgb,
            width=border_width
        )

        # inset axes (icon_size fraction wide)
        ax_ins = ax.inset_axes([x, y - icon_size/2, icon_size, icon_size],
    ...
    ax.axis('off')
    plt.title("Mythic+ Spec Popularity Tier List", color='white', fontsize=SUBTITLE_SIZE, pad=20)
    plt.tight_layout()
    ...
    post_data = {
        ...
        "total_runs": humanize_number(sum(int(r.get('total_runs', 0)) for r in dungeon_runs_per_level)),
    }
    print(post_data)
    client = get_openai_client(api_key)
    post = generate_post_text(client, post_data, url)
...
def create_spec_popularity_by_level(output_dir, donesocials, api_key, url, season, icon_size=0.4):
    """
    ...
    for x_frac, sid in zip(x_fracs, ordered):
        icon_file = os.path.join(
            ICON_DIR,
            f"{spec_lookup[str(sid)]['SpellIconFileId']}.jpg"
        )
    ...
    if max_keylevel is not None:
        ...
        for sid, cnt in runs_at_max.items():
            top_specs.append({
                ...
                'className': class_lookup[str(spec_lookup[str(sid)]['classID'])]['name'],
                'count': cnt
            })
    ...
    post_data = {
        ...
        "highest_specs": [f"{spec['specName']} - {spec['className']}" for spec in top_specs],
    }
    print(post_data)
    client = get_openai_client(api_key)
    post = generate_post_text(client, post_data, url)
...
def create_spec_popularity_vs_performance(output_dir, donesocials, api_key, url, season):
    ...
    raw_points = create_spec_scatter(spec_upgrades, spec_lookup, class_lookup, highest_run)
...
def compute_dungeon_score_from_rows(dungeon_rows,
                                    w_depleted=-1,
                                    w_1=1,
                                    w_2=2,
                                    w_3=3):
...
def create_dungeon_tierlist(output_dir, donesocials, api_key, url, season,
                            icon_size=0.4):
    """
    ...
    for t in tiers:
        ...
        ax.text(
            -0.05 * max_x, y, f"{t}-Tier",
            va='center', ha='right',
            fontsize=SMALL_SIZE, fontweight='bold',
            color=tier_colors[t], zorder=1
        )
    ...
    for _, row in df.iterrows():
        ...
        text_ins.text(
            0.5, 1.0,                   # x=center, y=top of this box
            dungeon_name,
            va='top', ha='center',
            color='white',
            fontsize=VERY_SMALL_SIZE,
...
def compute_panel_width(draw, blocks, font, icon_sz, text_offset, pad):
    ...
    for blk in blocks:
        for t in blk:
            ...
            label = f"{name} [{t['pick_rate']:.1f}%]" if t['pick_rate'] < 100 else f"{name} [100%]"
...
def createSpecOverview(output_dir, donesocials, api_key, url, spec_id, season):
    ...
    with closing(databaseConnector.get_connection()) as conn:
        cursor = conn.cursor()
        spec_upgrade_counts = databaseConnector.fetch_spec_upgrade(conn, cursor, spec_id, season)
        play_count = 0
        ...
        # hero tree picks
        hero_trees_raw = databaseConnector.fetch_hero_tree_overview(conn, cursor, spec_id, season)
        hero_trees = []
        ...
        # runs
        highest = get_run_data(
            False, spec_id, season
        )
        highest_run = create_MplusImage(highest, "highest_run", donesocials, False, False, False)
        spec_talent_overview = databaseConnector.fetch_spec_talent_overview(conn, cursor, spec_id, season)
        class_talent_overview = databaseConnector.fetch_class_talent_overview(conn, cursor, spec_id, season)
    ...
    # header
    class_color = (int(class_meta["color"]["r"]), int(class_meta["color"]["g"]), int(class_meta["color"]["b"]))
    draw.text((50,30), name_text, font=font_big, fill=class_color , stroke_width=2, stroke_fill=(0, 0, 0))
    ...
    fig, ax = plt.subplots(
        figsize=(WIDTH/DPI, key_upgrades_panel_h/DPI),
        dpi=DPI
    )
    ...
    colors = [RARITY_COLORS['Depleted'],RARITY_COLORS['Uncommon'],RARITY_COLORS['Epic'],RARITY_COLORS['Legendary']]
    for tier,col in zip(upgrade_counts,colors):
        ...
        txt = ax.text(
            ...
            va='center', ha='center',
            color='white', fontsize=VERY_SMALL_SIZE,
            fontweight='bold'
        )
    ...
    plt.savefig(buf,facecolor=fig.get_facecolor()); plt.close(fig)
    panel1 = Image.open(buf)
    canvas.paste(panel1, (0, HEIGHT - key_upgrades_panel_h))
    draw.text((60,105),f"{humanize_number(play_count)} total runs",font=font_sm, fill=class_color , stroke_width=2, stroke_fill=(0, 0, 0))
    ...
    for section, data in (('class_talents', class_talent_overview),
    ...
    # sort by pick_rate
    class_talents_sorted = sorted(talents['class_talents'], key=lambda x: x['pick_rate'])
    ...
    for (label, best, worst, bc, wc), (pw, ph) in zip(panels, panel_sizes):
        ...
        # best block
        for t in best:
            img = Image.open(os.path.join(ICON_DIR, f"{t['icon']}.png"))\
                    .convert("RGBA")\
                    .resize((icon_sz, icon_sz), Image.LANCZOS)
            canvas.paste(img, (x+pad, y), img)
            name = t['name'] if len(t['name']) < 40 else t['name'][:17] + '...'
            block_txt = f"{name} [{t['pick_rate']:.1f}%]" if t['pick_rate'] < 100 else f"{name} [100%]"
            draw.text(
    ...
    draw.text((0,0),f"Updated: {format_timestamp(upd*1000)}",font=font_sm,fill='gray')
...
def create_socials_post(donesocials, api_key, url):
    ...
    for spec_id in specs or ["62"]:
        def make_spec_gen(sid):
            return lambda: createSpecOverview(OUTPUT_DIR, donesocials, api_key, url, sid, current_season_id)
        spec_generators.append(make_spec_gen(spec_id))

    # Other generators
    def gen_dungeon_tier(): return create_dungeon_tierlist(OUTPUT_DIR, donesocials, api_key, url, current_season_id)
    def gen_spec_pop_vs_perf(): return create_spec_popularity_vs_performance(OUTPUT_DIR, donesocials, api_key, url, current_season_id)
    def gen_dungeon_pop_vs_ease(): return create_dungeon_popularity_vs_ease(OUTPUT_DIR, donesocials, api_key, url, current_season_id)
    def gen_overall_spec_popularity(): return create_overall_spec_popularity(OUTPUT_DIR, donesocials, api_key, url, current_season_id)
    def gen_spec_pop_by_level(): return create_spec_popularity_by_level(OUTPUT_DIR, donesocials, api_key, url, current_season_id)
